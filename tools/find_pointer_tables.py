...
def main() -> int:
    ...
    def section_class(secname):
        """'symbolic' (already farptr/dw Label), 'actionable' (raw db / dw $hex), or
        'generated' (no raw data lines -- macro/INCBIN, e.g. the SOUND_DRIVER banks; the
        table bytes don't come from editable db in this file)."""
        f = s2f.get(secname)
        if not f:
            return "generated"
        ...
        if re.search(r'\bfarptr\b|\bdw\s+[A-Za-z_]', body):
            return "symbolic"
        if re.search(r'^\s*(db|dw)\s+\$', body, re.M):
            return "actionable"
        return "generated"
    ...
    def record_run(bank, start_addr, fmt, recs, secname):
        ...
        cls = section_class(secname)
        score = anchored * 3 + n + (5 if cls == "actionable" else 0)
        runs.append({
            "bank": bank, "addr": start_addr, "fmt": fmt, "n": n,
            "targets": targets, "sec": secname, "file": s2f.get(secname, "?"),
            "anchored": anchored, "cls": cls, "actionable": cls == "actionable",
            "score": score,
        })
    ...
    actionable = [r for r in runs if r["cls"] == "actionable"]
    symbolic = [r for r in runs if r["cls"] == "symbolic"]
    generated = [r for r in runs if r["cls"] == "generated"]
    ...
    for r in actionable:
        ...
        out.append("")
    out.append(f"## Already symbolic ({len(symbolic)}) — section uses farptr/dw Label\n")
    for r in symbolic:
        out.append(f"- ${r['bank']:02x}:${r['addr']:04x} {r['n']}x {r['fmt']} "
                   f"`{r['sec']}` ({Path(r['file']).name})")
    out.append(f"\n## Generated, not loose db ({len(generated)}) — macro/INCBIN section "
               "(e.g. SOUND_DRIVER); table bytes aren't editable here\n")
    for r in generated:
        out.append(f"- ${r['bank']:02x}:${r['addr']:04x} {r['n']}x {r['fmt']} "
                   f"`{r['sec']}` ({Path(r['file']).name})")
    ...
    print(f"pointer-tables: {len(runs)} runs ({len(actionable)} actionable, "
          f"{len(symbolic)} symbolic, {len(generated)} generated) -> "
