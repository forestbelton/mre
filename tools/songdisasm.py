...
def emit_bank(rom, bank):
    items, labels, table, dead = disasm_bank(rom, bank)
    n_entries = len({a for a in items if isinstance(items[a], TableEntry)})
    ids_lo = 0x00 if bank == 0x3f else 0x2f
    # map descriptor addr -> sound id (lowest table index pointing at it)
    descr_id = {}
    for i in range(n_entries):
        tgt = items[BASE + 2*i].target
        descr_id.setdefault(tgt, ids_lo + i)

    L = []
    L.append(f"; Auto-generated by tools/songdisasm.py -- do not hand-edit blindly;")
    L.append(f"; regenerate with `python3 tools/songdisasm.py --emit {bank:02x}`.")
    L.append(f"; Song/SFX data for sound driver bank ${bank:02x}. See include/snd_song.inc.")
    L.append("")
    L.append(f'SECTION "snd_data_{bank:02x}", ROMX[$4b00], BANK[${bank:02x}]')
    L.append("")
    L.append(f"{lname(bank, BASE)}:\t\t; $4b00 song pointer table ({n_entries} entries)")
    ...
    while i < len(addrs):
        ...
        it = items[a]
        if isinstance(it, TableEntry):
            # compress runs of identical default pointers with REPT
            j = i
            while j < len(addrs) and isinstance(items[addrs[j]], TableEntry) \
                    and items[addrs[j]].target == it.target:
                j += 1
            run = j - i
            if run >= 4:
                idc = descr_id.get(it.target)
                L.append(f"\tREPT {run}  ; ids ${ids_lo + i:02x}-${ids_lo + j - 1:02x} -> "
                         f"id ${idc:02x} ({'silence' if idc == ids_lo and it.target==BASE+0x200 else 'default'})")
                L.append(f"\t\ts_songptr {lname(bank, it.target)}")
                L.append(f"\tENDR")
            else:
                for k in range(i, j):
            ...
            i = j
            continue
        # label line (with role banner)
        if a in labels:
            if isinstance(it, Descriptor):
                sid = descr_id.get(a, '?')
                L.append("")
                L.append(f"; ===== sound id ${sid:02x} =====" if isinstance(sid, int)
                         else "; ===== song =====")
            tag = ""
            if a in dead:
                tag = "\t; unused/dead pattern"
...
if __name__ == '__main__':
    ...
    ap.add_argument('--emit', metavar='BANK', help='emit RGBDS source for bank (3f/3e) to stdout')
    args = ap.parse_args()
    rom = Rom()
    if args.emit:
