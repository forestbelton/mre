...
- **bits 5-6** — **stereo panning** (NR51/`rAUDTERM` via `$43ff/$4401/$4403`),
  _not_ an octave shift. The handler shifts a per-channel L/R enable mask into
...
These ids are the `SOUND` enum in `include/sound/id.inc` (built with
...
| `$28`                               | BGM  | **BGM silence / stop** — its data is a 17-byte `rest; goto self` loop on all 4 channels (the BGM analogue of `$00`). Most-used (22 sites) because scenes play it to turn music off; despite being the boot/most-played BGM id it is _not_ a musical track. |
...
- **Done — editable source:** the whole song-data region (`$4b00-$7ffe`, both
  ...
  macros in `include/sound/id.inc` (note columns, octave, envelopes,
